...
**design_docs.md**
- Complete technical requirements and user stories for the Email Agent
- Database schema, API endpoints, tech stack specifications
...
1. ANALYZE
   - Read design_docs.md for requirements
...
- Use established UI frameworks (shadcn/ui)
...
2. Prioritize based on design_docs.md milestones
3. For each task:
...
## Current Project: Email Agent for Business Brokers

### Tech Stack
- **Backend**: FastAPI (Python 3.11+), PostgreSQL with pgvector, Celery/RQ
- **Frontend**: Next.js 14, TypeScript, Tailwind CSS, shadcn/ui
- **Integrations**: Gmail API, OpenAI GPT-4.x, S3-compatible storage
- **Hosting**: Backend on Railway/Fly.io, Frontend on Vercel, Postgres managed

### Key Architecture Decisions
1. **Monolith First**: Single repo, simple deployment for MVP
2. **Batch Processing**: Email responses sent in configurable windows to mimic human
3. **Vector Search**: pgvector in same Postgres instance (not separate service)
4. **Trust & Control**: Broker can review/override all agent responses
5. **Escalation**: Agent escalates uncertain/risky questions instead of guessing

### Development Phases (6 weeks)
- **Weeks 1-2**: Auth, settings, listings CRUD, database setup
- **Week 3**: Email ingestion, document upload, vector embeddings
- **Week 4**: Agent core (LLM integration, tools, batch processing)
- **Week 5**: NDA gating, Calendly integration, broker overrides
- **Week 6**: Hardening, logging, monitoring, pilot testing

### Critical Success Factors
1. **Email accuracy**: Agent must answer correctly or escalate (no hallucinations)
2. **NDA compliance**: Never share sensitive info without signed NDA
3. **Broker trust**: Dashboard transparency and override capability
4. **Human-like timing**: Batch windows prevent bot-like instant responses

See `design_docs.md` for complete technical requirements and user stories.
